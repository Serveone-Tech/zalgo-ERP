...
export default function AssignmentsPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createMutation.mutate({
      ...
      dueDate: fd.get("dueDate") ? new Date(fd.get("dueDate") as string).toISOString() : null,
    });
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Create and track assignments for batches</p>
        </div>
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button className="rounded-xl shadow-md shadow-primary/20" data-testid="button-add-assignment">
          ...
          <DialogContent className="sm:max-w-md rounded-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-4">
              ...
              <div className="space-y-2">
                <Label>Title *</Label>
                <Input name="title" required className="rounded-xl" placeholder="Assignment title" />
              </div>
              <div className="space-y-2">
                <Label>Description</Label>
                <Textarea name="description" className="rounded-xl" placeholder="Instructions or details..." />
              </div>
              <div className="space-y-2">
                <Label>Due Date</Label>
                <Input name="dueDate" type="datetime-local" className="rounded-xl" />
              </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center p-16 bg-card rounded-2xl border border-dashed border-border">
          ...
          <p className="text-sm text-muted-foreground/60 mt-1">Create your first assignment</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {assignments?.map((a: any) => {
            ...
            return (
              <div key={a.id} className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="font-semibold text-foreground mb-1 line-clamp-1">{a.title}</h3>
