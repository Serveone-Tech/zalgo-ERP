...
export default function ReportsPage() {
  ...
  const totalPending = pendingInstallments.reduce(
    (s: number, i: any) => s + (i.amount - (i.paidAmount ?? 0)),
    0,
  );
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Overview of all modules and key metrics
        </p>
      </div>
      ...
      <div>
        ...
        {/* Net Balance */}
        <div
          className={`mt-4 rounded-2xl p-5 border shadow-sm flex items-center justify-between ${netBalance >= 0 ? "bg-primary/5 border-primary/20" : "bg-destructive/5 border-destructive/20"}`}
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${netBalance >= 0 ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {Object.entries(incomeByCategory)
                ...
                .map(([cat, amt]: any) => {
                  const maxAmt = Math.max(
                    ...(Object.values(incomeByCategory) as number[]),
                  );
          ...
          <div>
            ...
            <div className="bg-card rounded-2xl border border-border/50 shadow-sm p-5 space-y-3">
              {Object.entries(expenseByCategory)
                ...
                .map(([cat, amt]: any) => {
                  const maxAmt = Math.max(
                    ...(Object.values(expenseByCategory) as number[]),
                  );
                  const pct = maxAmt > 0 ? (amt / maxAmt) * 100 : 0;
                  return (
                    <div key={cat} className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="bg-card rounded-2xl border border-border/50 shadow-sm p-5 space-y-3">
              {Object.entries(paymentModes)
                ...
                .map(([mode, amount]: any) => {
                  const maxAmt = Math.max(
                    ...(Object.values(paymentModes) as number[]),
                  );
        ...
        {dashboard?.courseEnrollments &&
          dashboard.courseEnrollments.filter((c: any) => c.studentCount > 0)
            .length > 0 && (
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-3">
                Course Enrollment
                ...
                {dashboard.courseEnrollments
                  ...
                  .map((ce: any) => {
                    const maxCount = Math.max(
                      ...dashboard.courseEnrollments.map(
                        (c: any) => c.studentCount,
                      ),
                    );
                    const pct =
                      maxCount > 0 ? (ce.studentCount / maxCount) * 100 : 0;
                    return (
                      <div
                        key={ce.courseName}
