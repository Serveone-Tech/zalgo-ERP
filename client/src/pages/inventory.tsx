    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Manage institute assets and stocks
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-card rounded-2xl p-4 border border-border/50 shadow-sm">
          ...
          <p className="text-2xl font-bold text-destructive mt-1">
            {items?.filter((i: any) => i.quantity < 5).length || 0}
          </p>
        </div>
        <div className="bg-card rounded-2xl p-4 border border-border/50 shadow-sm">
          <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">
          ...
          <p className="text-2xl font-bold text-destructive mt-1">
            {items?.filter((i: any) => i.quantity === 0).length || 0}
          </p>
        </div>
      </div>
                        ...
                        {canEdit && (
                          <Button
                            ...
                            onClick={() => {
                              setEditItem(item);
                              setEditBranchId(
                                item.branchId ? String(item.branchId) : "",
                              );
                            }}
                            data-testid={`btn-edit-inventory-${item.id}`}
                          >
      ...
      {editItem && (
        <Dialog open={!!editItem} onOpenChange={() => setEditItem(null)}>
          <DialogContent className="sm:max-w-md rounded-2xl">
            <DialogHeader>
              <DialogTitle>Edit — {editItem.itemName}</DialogTitle>
            </DialogHeader>
